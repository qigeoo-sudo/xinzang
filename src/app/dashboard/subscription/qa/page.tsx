  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: [
          '不会。只有导师分身真正生成的回复才计 1 轮；命中安全提醒、超出分身能力范围等系统固定提示不扣减任何额度，也不会计入本期、今日或加榨包的用量，聊天框上的数字保持不变。',
        ],
      },
    ],
  },
    ...
    items: [
      {
        q: '什么是加榨包？',
        a: [
          '加榨包是按轮次购买的加购额度，￥19.9 含 10 轮次。它不是会员：不授予会员身份、一次购买永不过期、用完再续，会员和非会员都可以购买，可重复购买。',
      ...
      },
      {
        q: '可以一次购买多个加榨包吗？有折扣吗？',
        a: [
          '可以。在加榨包卡片上用 − / ＋ 选择购买数量（单笔 1-99 个），每个含 10 轮次，一次下单、一次支付，所有轮次立即到账。',
          '批量折扣按整单数量计算：一次购买 5-9 个打 9 折，买满 5 个折后 ￥89；一次购买 10 个及以上一律 8.5 折，买满 10 个折后 ￥169。折后总价向下取整到元，例如买 10 个原价 ￥199，8.5 折为 ￥169.15，实付 ￥169；买 20 个实付 ￥338。',
      ...
      {
        q: '聊天框上方的“本期 40/60 · 今日 12/15 · 加榨包 0/10”是什么意思？',
        a: [
          '这是三个相互独立的额度，每个都按“已用 / 总额”显示：本期是当前订阅周期内的总轮次（非会员显示“免费试用 0/3”）；今日是最近 24 小时滚动窗口内的用量；加榨包是历次购买累计的加购轮次，只有买过加榨包才会显示这一段。',
          '三个池子各自计数，任何一个池子的已用量都不会超过它自己的总额。本期和今日都没满时消耗会员（或试用）额度；其中任意一个满了就改消耗加榨包，此时本期和今日的数字保持不动；加榨包也用完时，会引导你开通会员或再次购买加榨包。',
          '免费导师分身的对话不消耗任何额度。',
        ],
      },
    ],
